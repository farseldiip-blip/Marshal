/* =========================================================
   functions/paymob.js — Paymob (Legacy Accept API) provider.
   ---------------------------------------------------------
   SECURITY:
...
// Minimal JSON POST using Node built-ins (no extra dependency).
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            return reject(
              new Error("paymob_http_" + res.statusCode + " " + data)
            );
          }
          resolve(json);
        });
...
// Convert a major-unit amount (e.g. 120.50) to integer cents.
...
// Build a safe billing object. Paymob requires these fields; we
// substitute "NA" for anything missing (allowed by Paymob).
...
const paymob = {
  ...
  // Create a hosted-checkout session. Returns ONLY safe data.
  async createSession({ bookingId, amount, currency, email, booking }) {
    ...
    // Safe hosted iframe URL — the ONLY thing the client needs.
    const checkoutUrl =
    ...
    return {
      checkoutUrl,
      clientToken: null,
      // providerRef = paymob order id (stable per attempt).
      providerRef: String(order.id),
      // Keep the bookingId for webhook matching (we pass it round-trip).
      metadata: { bookingId, orderId: String(order.id) }
    };
  },

  // Verify the Paymob transaction webhook/callback using HMAC.
  // Returns a normalized event: { status, bookingId, providerRef,
  //   amountCents, currency, last4, method } or throws.
  verifyWebhook(req) {
    const hmacSecret = required("PAYMOB_HMAC_SECRET");

    // Paymob may send JSON body or form-encoded; normalize to object.
    ...
    const o = obj.obj || obj; // Paymob wraps real data in `obj`
    ...
    return {
      status,
      bookingId: merchantOrderId || orderId, // we pass bookingId as merchant_order_id
      providerRef: orderId,
  ...
  // Refund a captured (paid) transaction. `providerRef` is the
  // paymob order id. Paymob refund requires the transaction id;
  // we issue refund against the order's last transaction.
...
// ---------------- HMAC verification ----------------
// Paymob HMAC fields, in EXACT order, concatenated as strings,
// hashed with HMAC-SHA512 (hex) using the HMAC secret.
const HMAC_FIELDS = [
  "amount_cents",
  "created_at",
  "currency",
